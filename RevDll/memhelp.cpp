...
bool memhelp::RenderDetour(uint8_t instruction, void* source, void* dest)
{
	DWORD oldProtect;
	uint8_t buffer[5];
...
bool memhelp::RenderNop(void* addr, uint32_t count)
{
	DWORD oldProtect;
...
bool memhelp::WriteMem(void* addr, void* data, uint32_t count)
{
	DWORD oldProtect;
	if (!VirtualProtect(addr, count, PAGE_EXECUTE_READWRITE, &oldProtect))
		return false;
...
uint8_t* memhelp::ReadMem(void* addr, uint32_t count)
{
